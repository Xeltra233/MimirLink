...
test('buildChatRuntimePreview returns structured sources, messages, and effective binding', async () => {
    ...
    assert.equal(preview.messages[3].role, 'user');
    assert.equal(preview.messages[3].content, '你好');
    assert.equal(preview.messages[4].role, 'assistant');
    assert.equal(preview.messages[4].content, '角色助手预填充');
...
test('build exposes runtime sources and message trace for backend-composed prompt', async () => {
    ...
    assert.equal(built.messageTrace[3].sourceSlots.includes('user_input'), true);
    assert.equal(built.messageTrace[4].sourceSlots.includes('assistant_prefill'), true);
    ...
    assert.equal(built.messageTrace[4].sourceIds.includes('assistant-prefill'), true);
